...
PLEASE NOTE:  If you use Ring.com or the Ring App, this will sync to ST, but only after the Ring Device handler polls Ring to check on the status (every 5 min, or however often you set you device handler to poll).  So while it will sync ST SHM to Ring, it will take some time.  Do not rely on it for critical ST home automations if you change Ring from their app and not ST.
